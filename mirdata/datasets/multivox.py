...
INDEXES = {
    ...
    "1.0": core.Index(
        ...
        url=(
            "https://zenodo.org/records/18203441/files/"
            "mirdata_multivox_index_1.0.json?download=1"
        ),
...
class Track(core.Track):
    ...
    Cached Properties (Audio - loaded lazily on first access):
        ...
        audio_360 (tuple or None): 360° camera audio ``(np.ndarray[2, N], sample_rate)`` or None
    ...
    def audio_ortf_l(self) -> Optional[Tuple[np.ndarray, float]]:
    ...
    @core.cached_property
    def audio_ortf_r(self) -> Optional[Tuple[np.ndarray, float]]:
    ...
    @core.cached_property
    def audio_ortf_stereo(self) -> Optional[Tuple[np.ndarray, float]]:
        ...
        Returns:
            ...
            * float - sample rate
        """
        l_audio, l_sr = self.audio_ortf_l if self.audio_ortf_l_path else (None, None)
        r_audio, r_sr = self.audio_ortf_r if self.audio_ortf_r_path else (None, None)

        if l_audio is None or r_audio is None:
            return None
        ...
        # mypy: l_sr and r_sr are not None here because both audio arrays exist
        assert l_sr is not None
    ...
    def audio_360(self) -> Optional[Tuple[np.ndarray, float]]:
        ...
        Returns:
            * np.ndarray - stereo audio signal (2 channels), or None if not available
            * float - sample rate (typically 48000 Hz), or None if not available
        """
